...
export async function computeLastReadCfi() {
	try {
		log("computing last read cfi")
		viewportRect = constructDOMRect(Android.getViewportRect(DisplayUnit.CSS_PX));
		log("viewportRect: " + viewportRect)

		let filepath = Android.getBookName()
		log("filepath: " + filepath)
		let encoded_filepath = encodeURIComponent(filepath)
		log("encodedfilepath: " + encoded_filepath)

		let epub = new Epub("https://readium/books/" + encoded_filepath)
		log("epub: " + epub)

		await epub.opened
		let resource_pos = Android.getResourcePosition();
		log("resource pos: " + resource_pos)
		var spine = epub.spine.get(resource_pos)
		log("spine: " + spine)
